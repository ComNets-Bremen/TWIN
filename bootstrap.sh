...
DIRECTORIES=(bin bsl incoming logFiles outgoing) # specific directories for TWIN
CURRENT_USER=$(who | awk '{print $1}') # used to change ownership
LOG_FILES=(bsl incoming paramiko scheduler session TWIN) # log files to be in logFiles
...
createAdHocNetwork(){
	...
	ifconfig wlan0 down
	iwconfig wlan0 mode ad-hoc essid pi-adhoc channel 6 txpower 0dBm
	ifconfig wlan0 up
}
...
#------------------------------------------------------------------------#
echo
echo "TWIN: Step 8: Creating a backup and updating the network-interfaces file"
echo
